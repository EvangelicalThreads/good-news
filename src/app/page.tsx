...
export default function HomePage() {
  ...
  useEffect(() => {
    ...
    const fetchStreak = async () => {
      try {
        ...
        setLastStreakDate(
          data.streak_last_date ? new Date(data.streak_last_date).toLocaleDateString() : null,
        );
      } catch (err) {
        console.error(err);
      }
  ...
  async function generatePlan() {
    ...
    try {
      ...
      const data: { savedPlanId?: string; tasks?: AiPlanTask[]; error?: string } = await res.json();
      ...
      <section className="flex flex-col items-center text-center mb-8">
        ...
        {streak !== null && (
          <div className="mt-2 text-sm text-gray-600 font-medium flex gap-2 items-center">
            ...
            {lastStreakDate && <span className="text-gray-400">(last: {lastStreakDate})</span>}
          </div>
        )}
      </section>
        ...
        <div className="space-y-8">
          ...
          <section className="bg-white p-6 rounded-2xl shadow-xl space-y-4">
            ...
            >
              ...
              <span className="text-purple-600 font-bold">{planCollapsed ? '▼' : '▲'}</span>
            </div>
